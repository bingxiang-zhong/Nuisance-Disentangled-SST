...
from utils import Parameter, acoustic_power, cart2sph_np, load_file
...
class RandomTrajectoryDataset(Dataset):
    ...
    def __init__(
        ...
        if not isinstance(array, str):
            raise TypeError("array must be a fixed array name in this version.")
        if array == "random":
            raise ValueError("Random microphone arrays are disabled in this version.")
        if array not in ARRAY_SETUPS:
            raise ValueError(f"Unknown array setup: {array}")

        self.array = array
        self.array_setup = ARRAY_SETUPS[array]
    ...
    def __getitem__(self, idx):
        ...
        idx = self.shuffled_idxs[(idx % self.__len__() )]
        ...
        if self.cache and idx in self.cached_paths:
            ...
            mic_signals = self.simulate(acoustic_scene)
            ...
            # focus on 0 to 360 degree
            acoustic_scene_compact = {"DOAw":acoustic_scene['DOAw'] % 360,"vad":acoustic_scene['vad'],"fs":self.fs, "SNR":acoustic_scene['SNR']}
            return mic_signals,acoustic_scene_compact
    ...
    def get_batch(self, idx1, idx2):
        ...
        return (
            np.stack(mic_sig_batch),
            np.stack(acoustic_scene_batch)
        )
    ...
    def get_random_scene(self, idx):
        ...
        A = np.random.random(3) * np.minimum(
            Amax, 1.0      # for realman dataset using 0,5 instead of 1
        ...
        if np.random.random(1) < 0.25:  # used 0.75 for the traning should be 0.25
            traj_pts = np.ones((self.nb_points, 1)) * src_pos_ini
        

        traj_pts[:, 2] = mic_pos[0, 2] # set the same height of the trajectories
    ...
    def simulate(self, acoustic_scene):
        ...
        if "interferer_pos" in acoustic_scene:
            ...
            mic_signals = self.add_noise(mic_signals, acoustic_scene, acoustic_scene["noise_signal"], acoustic_scene["SNR"])
    ...
    def add_noise(self, input_mic_signal, acoustic_scene, noise_signal, snr_item):
        coeff = self.get_snr_coff(input_mic_signal, noise_signal, snr_item)
        if coeff is None:
            coeff = 1.0
        input_mic_signal += coeff * noise_signal[0:len(acoustic_scene["t"]), :]
